...
- 폴더 구조: 문서를 깔끔하게 정리
- 자동저장: 30초마다 자동 저장
  - 마지막 저장 후 30초가 지나면 변경된 내용을 자동으로 저장합니다.
  - 저장 중에는 상단에 "저장 중...", 완료되면 "저장됨 (방금 전)" 같은 상태 표시가 나타납니다.
  - 저장은 문서함(서버/클라우드)에 반영되며, 새로고침하거나 다른 기기에서 열어도 최신 내용을 확인할 수 있습니다.
  - Ctrl+S로 즉시 저장하면 자동저장 타이머가 그 시점부터 다시 시작됩니다.
